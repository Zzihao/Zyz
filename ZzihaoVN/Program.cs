...
namespace Zzihao
{
    public class Vayne
    {
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            ...
            menu = new Menu("Z字号定制VN", "Z字号定制VN", true).SetFontStyle(System.Drawing.FontStyle.Regular, Color.Red);
            //Orbwalker
           ...
           // emenu.AddItem(new MenuItem("Emod", "E的模式（0为新，1为旧）"))
            ...
            emenu.AddItem(new MenuItem("EF", "Z字号E闪").SetValue(true).SetFontStyle(System.Drawing.FontStyle.Regular, Color.Pink));
            emenu.AddItem(
        ...
        //    qss = menu.SubMenu("净化/水银").AddSubMenu(new Menu("解控设定", "使用净化物品"));
            ...
            menu.AddItem(new MenuItem("Emod", "E的模式").SetValue(new StringList(new[] {  "综合（推荐）","准确", "激进" })));
  ...
  //          {
    //            qss.AddItem(new MenuItem(buffs[i].ToString(), buffs[i].ToString()).SetValue(true));
      //      }
        ...
        {
            var antigapcloserEnabled = emenu.Item("Gap_E").GetValue<bool>();
            var endPosition = gapcloser.End;

            if (!antigapcloserEnabled || !E.IsReady() || !gapcloser.Sender.IsValidTarget() ||
        ...
        /*
              private static void OnEnemyGapcloser(ActiveGapcloser gapcloser, SpellSlot slot)
              {
                              if (gapcloser.Sender.IsValidTarget(E.Range) &&
                                  emenu.Item("Gap_E").GetValue<bool>()
                                  && (E.IsReady()))
                              {
